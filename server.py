...
@mcp.tool(description="-Fetch historical stock prices by symbol, exchange, period, and time interval\neg) SBIN stock price in NSE for last 3 days with time interval of 1 hour or whenever such a task is called where we need the price data of stock for example if user asks to simply tell yearly gains on a stock, use this tool for any query that requires the prices of a stock for any computation")
async def stock_time_series(
    ...
    period: Annotated[str, Field(description="Data period,preferably convert any input to number of DAYS and add'd' at end like examples ahead, e.g. '1d','5d','60d'")] = "2d",
    interval: Annotated[str, Field(description="Data interval maximum interval is 5days which corresponds to '5d' if user calls for a bigger interval than 5 days automatically reser it to '5d', e.g. '1m','5m','15m','30m','1h','1d'")] = "1h",
    ...
    try:
        ...
        # 1) Build your datetime list with IST offset (no string conversions here)
        times_dt = []
